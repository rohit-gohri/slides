...
The code that runs on every push/event is termed as Pipelines. Most CI Servers expects a config defined in a .yaml file.
...
I have added yaml configs for various CI providers in one single repo. You can check how they are setup later.
...
```yaml 5:7 title="Env variable overrides" file=../static/code/.gitlab-ci.yml
```
...
2. Specify any env variable overrides
